...
import timeit as t
import global_data_total_percent_PR_HISP
import numpy as np
import glob
import collections
import csv
...
def create_row_dataframe(PUMA=None, NATIVITY=None, AGEP=None, SCHL=None, SEX=None, WKHP=None, ESR=None, PINCP=None,
    ...
    Population_mf_t,Population_m,Population_f = 0,0,0
    ...
    # Filter out for PR_Hispanic born in PR/Puerto rican born in PR
    ...
    # Filter out for NB_Mainland_PR/Puerto rican born in mainland
    if NATIVITY is 1 and HISP is 1 and POBP is 1:
        p_data1 = p_data1[p_data1['HISP'] == 3]
        p_data1 = p_data1[p_data1['POBP'].isin(range(1, 56 + 1))]

    # Filter out for PR_POC in PR
    if NATIVITY is 1 and HISP is 1 and RAC1P is 4 and POBP72 is 1:
        p_data1 = p_data1[p_data1['HISP'].isin(range(2, 24 + 1))]
        p_data1 = p_data1[p_data1['RAC1P'].isin(range(2, 8 + 1))]
        p_data1 = p_data1[p_data1['POBP'] == 72]

    # Filter out for PR_White in PR
    if NATIVITY is 1 and HISP is 3 and RAC1P is 5 and POBP72 is 1:
        p_data1 = p_data1[p_data1['HISP'].isin(range(2, 24 + 1))]
        p_data1 = p_data1[p_data1['RAC1P'] == 1]
        p_data1 = p_data1[p_data1['POBP'] == 72]
    ...
    for i in p_data1.index:
        if p_data1.at[i, 'SEX'] == 1:
            Population_m += p_data1.at[i, 'PWGTP']


    #  Female
    for i in p_data1.index:
        if p_data1.at[i, 'SEX'] == 2:
            Population_f += p_data1.at[i, 'PWGTP']

    # ------------------------------------------------------------------------------------------------------------------

    #Total Population
    Population_mf_t = Population_m + Population_f

    list_to_return = [Population_mf_t,
                      Population_m, Population_f]
    
    return list_to_return


def Total_Population(NATIVITY = None, PATH = '/', RAC1P=None, HISP=None,POBP=None,POBP72=None,name=None):
    ...
    print '----------------- For Total Geo'
    l = create_row_dataframe(NATIVITY=NATIVITY,RAC1P=RAC1P,HISP=HISP,POBP=POBP,POBP72=POBP72)
    l.insert(0, "Total Geo")
    full_list.append(l)
    # For all the PUMA counties.
    for p in global_data_total_percent_PR_HISP.PUMA_Counties:
        print '----------------- For PUMA: ', puma_county(p)
        l = create_row_dataframe(PUMA=p,NATIVITY=NATIVITY,RAC1P=RAC1P,HISP=HISP,POBP=POBP,POBP72=POBP72)
    ...
    if NATIVITY is 1 and POBP is None and RAC1P is None and HISP is None:
        global_data_total_percent_PR_HISP.NB_ALL_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.NB_ALL_p.to_csv(PATH + 'step_2/' + 'NB_ALL_total_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 2 and HISP is None and RAC1P is None and POBP is None and POBP72 is None:
        global_data_total_percent_PR_HISP.FB_ALL_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.FB_ALL_p.to_csv(PATH + 'step_2/' + 'FB_ALL_total_percent.csv',
                                                          na_rep="#DIV/0!")
    if NATIVITY is 2 and HISP is 1 and RAC1P is None:
        global_data_total_percent_PR_HISP.FB_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.FB_Hispanic_p.to_csv(PATH + 'step_2/' + 'FB_Hispanic_total_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 2 and RAC1P is 1 and HISP is 1:
        global_data_total_percent_PR_HISP.FB_WNH_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.FB_WNH_p.to_csv(PATH + 'step_2/' + 'FB_WNH_total_percent.csv', na_rep="#DIV/0!")



    if NATIVITY is 1 and HISP is 1 and POBP72 is 1 and RAC1P is None:
        global_data_total_percent_PR_HISP.PR_Hispanic_p = get_percentage(df=result_df)

        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.PR_Hispanic_p.to_csv(PATH + 'step_2/' + 'PR_Hispanic_total_percent.csv', na_rep="#DIV/0!")



    if NATIVITY is 1 and POBP is 1 and HISP is None and RAC1P is None:
        global_data_total_percent_PR_HISP.NB_Mainland_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.NB_Mainland_p.to_csv(PATH + 'step_2/' + 'NB_Mainland_total_percent.csv', na_rep="#DIV/0!")




    if NATIVITY is 1 and POBP is 1 and HISP is 1 and RAC1P is None:
        global_data_total_percent_PR_HISP.NB_Mainland_PR_p = get_percentage(df=result_df)

        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.NB_Mainland_PR_p.to_csv(PATH + 'step_2/' + 'NB_Mainland_PR_total_percent.csv', na_rep="#DIV/0!")




    if NATIVITY is 2 and HISP is 1 and RAC1P is 2:
        global_data_total_percent_PR_HISP.FB_Hispanic_POC_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.FB_Hispanic_POC_p.to_csv(PATH + 'step_2/' + 'FB_Hispanic_POC_total_percent.csv', na_rep="#DIV/0!")
    # Filter for NB_WNH
    if NATIVITY is 1 and HISP is 1 and RAC1P is 1:
        global_data_total_percent_PR_HISP.NB_WNH_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.NB_WNH_p.to_csv(PATH + 'step_2/' + 'NB_WNH_total_percent.csv', na_rep="#DIV/0!")
    # Filter for NB_Hispanic
    if NATIVITY is 1 and HISP is 1 and RAC1P is None and POBP is None and POBP72 is None:
        global_data_total_percent_PR_HISP.NB_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.NB_Hispanic_p.to_csv(PATH + 'step_2/' + 'NB_Hispanic_total_percent.csv', na_rep="#DIV/0!")
    # Filter for FB_White Hispanic
    if NATIVITY is 2 and HISP is 2 and RAC1P is 3:
        global_data_total_percent_PR_HISP.FB_White_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.FB_White_Hispanic_p.to_csv(PATH + 'step_2/' + 'FB_White_Hispanic_total_percent.csv', na_rep="#DIV/0!")
    # Filter out for PR_POC in PR
    if NATIVITY is 1 and HISP is 1 and RAC1P is 4 and POBP72 is 1:
        global_data_total_percent_PR_HISP.PR_POC_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.PR_POC_p.to_csv(PATH + 'step_2/' + 'PR_POC_total_percent.csv', na_rep="#DIV/0!")
    #Filter out for PR_White in PR
    if NATIVITY is 1 and HISP is 3 and RAC1P is 5 and POBP72 is 1:
        global_data_total_percent_PR_HISP.PR_White_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent_PR_HISP.PR_White_p.to_csv(PATH + 'step_2/' + 'PR_White_total_percent.csv', na_rep="#DIV/0!")


def NB_ALL():
    print 'NB_ALL'
    PATH = 'data_PR_HISP/2016/NB_All/'
    Total_Population(NATIVITY=1, PATH=PATH,name='NB_ALL_total_population.csv')

def FB_ALL():
    print 'FB_ALL'
    PATH = 'data_PR_HISP/2016/FB_All/'
    Total_Population(NATIVITY=2, PATH=PATH,name='FB_ALL_total_population.csv')

def FB_Hispanic():
    print 'FB_Hispanic'
    PATH = 'data_PR_HISP/2016/FB_Hispanic/'
    Total_Population(NATIVITY=2, HISP=1, RAC1P=None, PATH=PATH,name='FB_Hispanic_total_population.csv')
...
def FB_WNH():
    ...
    Total_Population(NATIVITY=2, HISP=1, RAC1P=1, PATH=PATH,name='FB_WNH_total_population.csv')
...
def FB_Hispanic_POC():
    ...
    Total_Population(NATIVITY=2, HISP=1, RAC1P=2, PATH=PATH,name='FB_Hispanic_POC_total_population.csv')
...
def PR_Hispanic():
    ...
    Total_Population(NATIVITY=1, HISP=1,POBP72=1, PATH=PATH,name='PR_Hispanic_total_population.csv')
...
def NB_Mainland_PR():
    ...
    Total_Population(NATIVITY=1, POBP=1, HISP=1, PATH=PATH,name='NB_Mainland_PR_total_population.csv')
...
def NB_Mainland():
    ...
    Total_Population(NATIVITY=1, POBP=1, PATH=PATH,name='NB_Mainland_total_population.csv')
...
def NB_WNH():
    ...
    Total_Population(NATIVITY=1, HISP=1, RAC1P=1, PATH=PATH,name='NB_WNH_total_population.csv')
...
def NB_Hispanic():
    ...
    Total_Population(NATIVITY=1, HISP=1, RAC1P=None, POBP72=None, POBP=None, PATH=PATH,name='NB_Hispanic_total_population.csv')
...
def FB_White_Hispanic():
    ...
    Total_Population(NATIVITY=2, HISP=2, RAC1P=3, PATH=PATH,name='FB_WHite_Hispanic_total_population.csv')

def PR_POC():
    print 'PR_People of color'
    PATH = 'data_PR_HISP/2016/PR_POC/'

    Total_Population(NATIVITY=1, HISP=1, RAC1P=4,POBP72=1,PATH=PATH,name='PR_POC_total_population.csv')

def PR_White():
    print 'PR_White'
    PATH = 'data_PR_HISP/2016/PR_White/'

    Total_Population(NATIVITY=1, HISP=3, RAC1P=5,POBP72=1,PATH=PATH,name='PR_White_total_population.csv')
...
def get_percentage(df = None):
    ...
    for i in df.index:
        ...
        new_df.at[i, 'Population_m'] = ((df.at[i,'Population_m'] * (1.0)) / (df.at[i,'Population_m'] + df.at[i,'Population_f']))*100
        new_df.at[i, 'Population_f'] = ((df.at[i,'Population_f'] * (1.0)) / (df.at[i,'Population_f'] + df.at[i,'Population_m']))*100
...
def total_percentage(PATH=None):
    ...
    #FB_Hispanic/FB_All
    total_population_FB_Hispanic=pandas.read_csv('data_PR_HISP/2016/FB_Hispanic/step_1/FB_Hispanic_total_population.csv')
    total_population_FB_All = pandas.read_csv('data_PR_HISP/2016/FB_All/step_1/FB_ALL_total_population.csv')
    new_FB_Hispanic = pandas.DataFrame()
    for i in total_population_FB_Hispanic.index[146:]:
        new_FB_Hispanic.at[i,'puma'] = total_population_FB_Hispanic.at[i,'puma']
        new_FB_Hispanic.at[i,'Percentage_mf_t'] = ((total_population_FB_Hispanic.at[i,'Population_mf_t'])) * (100.0)/ (total_population_FB_All.at[i,'Population_mf_t'])
        new_FB_Hispanic.at[i,'Percentage_m'] = ((total_population_FB_Hispanic.at[i,'Population_m'])) * (100.0)/ (total_population_FB_All.at[i,'Population_m'])
        new_FB_Hispanic.at[i,'Percentage_f'] = ((total_population_FB_Hispanic.at[i,'Population_f'])) * (100.0)/ (total_population_FB_All.at[i,'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_FB_Hispanic.to_csv(PATH + 'Total_Population_Percentage/' + 'FB_Hispanic_total_population_percent.csv', na_rep="#DIV/0!")

    #FB_Hispanic_POC/FB_Hispanic
    total_population_FB_Hispanic_POC = pandas.read_csv('data_PR_HISP/2016/FB_Hispanic_POC/step_1/FB_Hispanic_POC_total_population.csv')
    total_population_FB_Hispanic = pandas.read_csv('data_PR_HISP/2016/FB_Hispanic/step_1/FB_Hispanic_total_population.csv')
    new_FB_Hispanic_POC = pandas.DataFrame()
    for i in total_population_FB_Hispanic_POC.index[146:]:
        new_FB_Hispanic_POC.at[i, 'puma'] = total_population_FB_Hispanic_POC.at[i, 'puma']
        new_FB_Hispanic_POC.at[i, 'Percentage_mf_t'] = ((total_population_FB_Hispanic_POC.at[i, 'Population_mf_t'])) * (100.0) / (total_population_FB_Hispanic.at[i, 'Population_mf_t'])
        new_FB_Hispanic_POC.at[i, 'Percentage_m'] = ((total_population_FB_Hispanic_POC.at[i, 'Population_m'])) * (100.0) / (total_population_FB_Hispanic.at[i, 'Population_m'])
        new_FB_Hispanic_POC.at[i, 'Percentage_f'] = ((total_population_FB_Hispanic_POC.at[i, 'Population_f'])) * (100.0) / (total_population_FB_Hispanic.at[i, 'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_FB_Hispanic_POC.to_csv(PATH + 'Total_Population_Percentage/' + 'FB_Hispanic_POC_total_population_percent.csv',na_rep="#DIV/0!")
    
    #PR_Hispanic/NB_All
    total_population_PR_Hispanic = pandas.read_csv('data_PR_HISP/2016/PR_Hispanic/step_1/PR_Hispanic_total_population.csv')
    total_population_NB_ALL = pandas.read_csv('data_PR_HISP/2016/NB_All/step_1/NB_ALL_total_population.csv')
    new_PR_Hispanic = pandas.DataFrame()
    for i in total_population_PR_Hispanic.index[146:]:
        new_PR_Hispanic.at[i, 'puma'] = total_population_PR_Hispanic.at[i, 'puma']
        new_PR_Hispanic.at[i, 'Percentage_mf_t'] = ((total_population_PR_Hispanic.at[i, 'Population_mf_t'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_mf_t'])
        new_PR_Hispanic.at[i, 'Percentage_m'] = ((total_population_PR_Hispanic.at[i, 'Population_m'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_m'])
        new_PR_Hispanic.at[i, 'Percentage_f'] = ((total_population_PR_Hispanic.at[i, 'Population_f'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_PR_Hispanic.to_csv(PATH + 'Total_Population_Percentage/' + 'PR_Hispanic_total_population_percent.csv',na_rep="#DIV/0!")
    
    #NB_Mainland_PR/NB_All
    total_population_NB_Mainland_PR = pandas.read_csv('data_PR_HISP/2016/NB_Mainland_PR/step_1/NB_Mainland_PR_total_population.csv')
    total_population_NB_ALL = pandas.read_csv('data_PR_HISP/2016/NB_All/step_1/NB_ALL_total_population.csv')
    new_NB_Mainland_PR = pandas.DataFrame()
    for i in total_population_NB_Mainland_PR.index[146:]:
        new_NB_Mainland_PR.at[i, 'puma'] = total_population_NB_Mainland_PR.at[i, 'puma']
        new_NB_Mainland_PR.at[i, 'Percentage_mf_t'] = ((total_population_NB_Mainland_PR.at[i, 'Population_mf_t'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_mf_t'])
        new_NB_Mainland_PR.at[i, 'Percentage_m'] = ((total_population_NB_Mainland_PR.at[i, 'Population_m'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_m'])
        new_NB_Mainland_PR.at[i, 'Percentage_f'] = ((total_population_NB_Mainland_PR.at[i, 'Population_f'])) * (100.0) / (total_population_NB_ALL.at[i, 'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_NB_Mainland_PR.to_csv(PATH + 'Total_Population_Percentage/' + 'NB_Mainland_PR_total_population_percent.csv',na_rep="#DIV/0!")
    
    #PR_POC/PR_Hispanic
    total_population_PR_POC = pandas.read_csv(
        'data_PR_HISP/2016/PR_POC/step_1/PR_POC_total_population.csv')
    total_population_PR_Hispanic = pandas.read_csv('data_PR_HISP/2016/PR_Hispanic/step_1/PR_Hispanic_total_population.csv')
    new_PR_POC = pandas.DataFrame()
    for i in total_population_PR_POC.index[146:]:
        new_PR_POC.at[i, 'puma'] = total_population_PR_POC.at[i, 'puma']
        new_PR_POC.at[i, 'Percentage_mf_t'] = ((total_population_PR_POC.at[i, 'Population_mf_t'])) * (
        100.0) / (total_population_PR_Hispanic.at[i, 'Population_mf_t'])
        new_PR_POC.at[i, 'Percentage_m'] = ((total_population_PR_POC.at[i, 'Population_m'])) * (100.0) / (
        total_population_PR_Hispanic.at[i, 'Population_m'])
        new_PR_POC.at[i, 'Percentage_f'] = ((total_population_PR_POC.at[i, 'Population_f'])) * (100.0) / (
        total_population_PR_Hispanic.at[i, 'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_PR_POC.to_csv(PATH + 'Total_Population_Percentage/' + 'PR_POC_total_population_percent.csv',
                           na_rep="#DIV/0!")
    
    #PR_White/PR_Hispanic
    total_population_PR_White = pandas.read_csv(
        'data_PR_HISP/2016/PR_White/step_1/PR_White_total_population.csv')
    total_population_PR_Hispanic = pandas.read_csv(
        'data_PR_HISP/2016/PR_Hispanic/step_1/PR_Hispanic_total_population.csv')
    new_PR_White = pandas.DataFrame()
    for i in total_population_PR_White.index[146:]:
        new_PR_White.at[i, 'puma'] = total_population_PR_White.at[i, 'puma']
        new_PR_White.at[i, 'Percentage_mf_t'] = ((total_population_PR_White.at[i, 'Population_mf_t'])) * (
            100.0) / (total_population_PR_Hispanic.at[i, 'Population_mf_t'])
        new_PR_White.at[i, 'Percentage_m'] = ((total_population_PR_White.at[i, 'Population_m'])) * (100.0) / (
            total_population_PR_Hispanic.at[i, 'Population_m'])
        new_PR_White.at[i, 'Percentage_f'] = ((total_population_PR_White.at[i, 'Population_f'])) * (100.0) / (
            total_population_PR_Hispanic.at[i, 'Population_f'])

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_PR_White.to_csv(PATH + 'Total_Population_Percentage/' + 'PR_White_total_population_percent.csv',
                      na_rep="#DIV/0!")
...
def run():
    ...
    #FB_Hispanic,FB_Hispanic_POC,FB_White_Hispanic,FB_WNH are all different
    #NB_All matches NB_Mainland, NB_Hispanic matches NB_Mainland_PR,NB_WNH is different
    #PR_Hispanic matches NB_Mainland_PR, PR_POC matches PR_White and NB_mainland, NB_All
    ...
    '''
    total_percentage(PATH='data_PR_HISP/2016/')
