...
import timeit as t
import global_data_edu_PR_HISP
import numpy as np
import glob
import collections
import csv
...
def create_row_dataframe(PUMA=None, NATIVITY=None, AGEP=None, SCHL=None, SEX=None, WKHP=None, ESR=None, PINCP=None,
    ...
    ENG_prof_m,ENG_prof_f = 0,0
    ENG_prof_mf_t = 0

    NO_ENG_prof_m,NO_ENG_prof_f = 0,0
    NO_ENG_prof_mf_t = 0,0
    ...
    for i in p_data1.index:
        if p_data1.at[i,'ENG'] in (range(1,2+1)) \
            and p_data1.at[i, 'SEX'] == 1:
            ENG_prof_m += p_data1.at[i, 'PWGTP']

    for i in p_data1.index:
        if p_data1.at[i,'ENG'] in (range(1,2+1)) \
                and p_data1.at[i, 'SEX'] == 2:
            ENG_prof_f += p_data1.at[i, 'PWGTP']

    for i in p_data1.index:
        if p_data1.at[i,'ENG'] in (range(3,4+1)) \
                and p_data1.at[i, 'SEX'] == 1:
            NO_ENG_prof_m += p_data1.at[i, 'PWGTP']

    for i in p_data1.index:
        if p_data1.at[i, 'ENG'] in (range(3,4+1)) \
                and p_data1.at[i, 'SEX'] == 2:
            NO_ENG_prof_f += p_data1.at[i, 'PWGTP']


    ENG_prof_mf_t = ENG_prof_m + ENG_prof_f
    NO_ENG_prof_mf_t = NO_ENG_prof_m + NO_ENG_prof_f

    Total_Population = ENG_prof_mf_t + NO_ENG_prof_mf_t
...
def ENG_Proficient(NATIVITY = None, PATH = '/', RAC1P=None, HISP=None,POBP=None,POBP72=None,name=None):
    ...
    for p in global_data_edu_PR_HISP.PUMA_Counties:
    ...
    if NATIVITY is 1 and POBP is None and RAC1P is None and HISP is None:
        global_data_edu_PR_HISP.NB_ALL_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.NB_ALL_p.to_csv(PATH + 'step_2/' + 'NB_ALL_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 2 and HISP is 1 and RAC1P is None:
        global_data_edu_PR_HISP.FB_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.FB_Hispanic_p.to_csv(PATH + 'step_2/' + 'FB_Hispanic_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 2 and RAC1P is 1 and HISP is 1:
        global_data_edu_PR_HISP.FB_WNH_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.FB_WNH_p.to_csv(PATH + 'step_2/' + 'FB_WNH_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 1 and HISP is 1 and POBP72 is 1 and RAC1P is None:
        global_data_edu_PR_HISP.PR_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.PR_Hispanic_p.to_csv(PATH + 'step_2/' + 'PR_Hispanic_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 1 and POBP is 1 and HISP is None and RAC1P is None:
        global_data_edu_PR_HISP.NB_Mainland_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.NB_Mainland_p.to_csv(PATH + 'step_2/' + 'NB_Mainland_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 1 and POBP is 1 and HISP is 1 and RAC1P is None:
        global_data_edu_PR_HISP.NB_Mainland_PR_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.NB_Mainland_PR_p.to_csv(PATH + 'step_2/' + 'NB_Mainland_PR_eng_prof_percent.csv', na_rep="#DIV/0!")
    if NATIVITY is 2 and HISP is 1 and RAC1P is 2:
        global_data_edu_PR_HISP.FB_Hispanic_POC_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.FB_Hispanic_POC_p.to_csv(PATH + 'step_2/' + 'FB_Hispanic_POC_eng_prof_percent.csv', na_rep="#DIV/0!")
    # Filter for NB_WNH
    if NATIVITY is 1 and HISP is 1 and RAC1P is 1:
        global_data_edu_PR_HISP.NB_WNH_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.NB_WNH_p.to_csv(PATH + 'step_2/' + 'NB_WNH_eng_prof_percent.csv', na_rep="#DIV/0!")
    # Filter for NB_Hispanic
    if NATIVITY is 1 and HISP is 1 and RAC1P is None and POBP is None and POBP72 is None:
        global_data_edu_PR_HISP.NB_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.NB_Hispanic_p.to_csv(PATH + 'step_2/' + 'NB_Hispanic_eng_prof_percent.csv', na_rep="#DIV/0!")
    # Filter for FB_White Hispanic
    if NATIVITY is 2 and HISP is 2 and RAC1P is 3:
        global_data_edu_PR_HISP.FB_White_Hispanic_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.FB_White_Hispanic_p.to_csv(PATH + 'step_2/' + 'FB_White_Hispanic_eng_prof_percent.csv', na_rep="#DIV/0!")
    # Filter out for PR_POC in PR
    if NATIVITY is 1 and HISP is 1 and RAC1P is 4 and POBP72 is 1:
        global_data_edu_PR_HISP.PR_POC_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.PR_POC_p.to_csv(PATH + 'step_2/' + 'PR_POC_eng_prof_percent.csv', na_rep="#DIV/0!")
    #Filter out for PR_White in PR
    if NATIVITY is 1 and HISP is 3 and RAC1P is 5 and POBP72 is 1:
        global_data_edu_PR_HISP.PR_White_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_edu_PR_HISP.PR_White_p.to_csv(PATH + 'step_2/' + 'PR_White_eng_prof_percent.csv', na_rep="#DIV/0!")


def NB_ALL():
    print 'NB_ALL'
    PATH = 'data_PR_HISP/2016/NB_All/'
    ENG_Proficient(NATIVITY=1, PATH=PATH,name='NB_ALL_eng_prof.csv')

def FB_Hispanic():
    print 'FB_Hispanic'
    PATH = 'data_PR_HISP/2016/FB_Hispanic/'

    ENG_Proficient(NATIVITY=2, HISP=1, RAC1P=None, PATH=PATH,name='FB_Hispanic_eng_prof.csv')
...
def FB_WNH():
    ...
    ENG_Proficient(NATIVITY=2, HISP=1, RAC1P=1, PATH=PATH,name='FB_WNH_eng_prof.csv')
...
def FB_Hispanic_POC():
    ...
    ENG_Proficient(NATIVITY=2, HISP=1, RAC1P=2, PATH=PATH,name='FB_Hispanic_POC_eng_prof.csv')
...
def PR_Hispanic():
    ...
    ENG_Proficient(NATIVITY=1, HISP=1, POBP72=1,RAC1P=None, PATH=PATH,name='PR_Hispanic_eng_prof.csv')
...
def NB_Mainland():
    ...
    ENG_Proficient(NATIVITY=1, POBP=1, PATH=PATH,name='NB_Mainland_eng_prof.csv')
...
def NB_Mainland_PR():
    ...
    ENG_Proficient(NATIVITY=1, POBP=1, HISP=1, PATH=PATH,name='NB_Mainland_PR_eng_prof.csv')
...
def NB_WNH():
    ...
    ENG_Proficient(NATIVITY=1, HISP=1, RAC1P=1, PATH=PATH,name='NB_WNH_eng_prof.csv')
...
def NB_Hispanic():
    ...
    ENG_Proficient(NATIVITY=1, HISP=1, RAC1P=None, POBP72=None, POBP=None, PATH=PATH,name='NB_Hispanic_eng_prof.csv')
...
def FB_White_Hispanic():
    ...
    ENG_Proficient(NATIVITY=2, HISP=2, RAC1P=3, PATH=PATH,name='FB_WHite_Hispanic_eng_prof.csv')

def PR_POC():
    print 'PR_People of color'
    PATH = 'data_PR_HISP/2016/PR_POC/'

    ENG_Proficient(NATIVITY=1, HISP=1, RAC1P=4,POBP72=1,PATH=PATH,name='PR_POC_eng_prof.csv')

def PR_White():
    print 'PR_White'
    PATH = 'data_PR_HISP/2016/PR_White/'

    ENG_Proficient(NATIVITY=1, HISP=3, RAC1P=5,POBP72=1,PATH=PATH,name='PR_White_eng_prof.csv')
...
def get_percentage(df = None):
    ...
    for i in df.index:
        ...
        new_df.at[i, 'ENG_prof_mf_t'] = ((df.at[i, 'ENG_prof_mf_t'] * (1.0)) / df.at[i,'Total_Population'])*100
        new_df.at[i, 'ENG_prof_m'] = ((df.at[i,'ENG_prof_m'] * (1.0)) / (df.at[i,'ENG_prof_m'] + df.at[i,'NO_ENG_prof_m']))*100
        new_df.at[i, 'ENG_prof_f'] = ((df.at[i,'ENG_prof_f'] * (1.0)) / (df.at[i,'ENG_prof_f'] + df.at[i,'NO_ENG_prof_f']))*100

        new_df.at[i, 'NO_ENG_prof_mf_t'] = ((df.at[i, 'NO_ENG_prof_mf_t'] * (1.0)) / df.at[i, 'Total_Population']) * 100
        new_df.at[i, 'NO_ENG_prof_m'] = ((df.at[i, 'NO_ENG_prof_m'] * (1.0)) / (df.at[i,'NO_ENG_prof_m'] + df.at[i,'ENG_prof_m'])) * 100
        new_df.at[i, 'NO_ENG_prof_f'] = ((df.at[i, 'NO_ENG_prof_f'] * (1.0)) / (df.at[i,'NO_ENG_prof_f'] + df.at[i,'ENG_prof_f'])) * 100
