...
import timeit as t
import global_data_total_percent
import numpy as np
import glob
import collections
import csv
...
def Total_Population(NATIVITY = None, PATH = '/', RAC1P=None, HISP=None,GRP11=None,name=None):
    ...
    if NATIVITY is 2 and RAC1P is None and HISP is None:
        global_data_total_percent.FB_ALL_p = get_percentage(df=result_df)
        make_sure_path_exists(PATH + 'step_2/')
        global_data_total_percent.FB_ALL_p.to_csv(PATH + 'step_2/' + 'FB_ALL_total_percent.csv', na_rep="#DIV/0!")
...
def total_percentage(PATH=None):
    ...
    total_population_f = total_population_df['Population_f'][146:].sum()
    for i in total_population_df.index[146:]:
        new_df.at[i,'puma'] = total_population_df.at[i,'puma']
        new_df.at[i,'Percentage_mf_t'] = ((total_population_df.at[i,'Population_mf_t'])) * (100.0)/ total_population
        new_df.at[i,'Percentage_m'] = ((total_population_df.at[i,'Population_m'])) * (100.0)/ total_population_m
        new_df.at[i,'Percentage_f'] = ((total_population_df.at[i,'Population_f'])) * (100.0)/ total_population_f

    make_sure_path_exists(PATH + 'Total_Population_Percentage/')
    new_df.to_csv(PATH + 'Total_Population_Percentage/' + 'FB_ALL_total_population_percent.csv', na_rep="#DIV/0!")
